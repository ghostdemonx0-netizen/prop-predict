...
def test_prime_prior_seasons_pitcher_branch(monkeypatch):
    # bypass on-disk caching so the test never touches the real .cache dir
    monkeypatch.setattr(backfill_history, "get_or_compute", lambda key, producer, *a, **k: producer())
    calls = []
    monkeypatch.setattr(fetch, "pitcher_events", lambda pid, yr: calls.append((pid, yr)) or [])
    n = backfill_history.prime_prior_seasons([7], 2026, batter=False)
    assert n == 2  # 1 pitcher x 2 prior seasons
    assert set(calls) == {(7, 2025), (7, 2024)}
