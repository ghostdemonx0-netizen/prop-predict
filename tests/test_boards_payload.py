...
def test_started_games_skipped():
    """build_boards_payload SKIPS started games — refresh_today freezes their boards
    from the pre-start record instead, so an in-game sub can't rewrite them."""
    ...
    assert boards["games"] == [], "started game must NOT be built fresh (it's frozen)"
